 ...
 * @notice UUPS upgrade of EscrowUpgradeable. Manages the full job lifecycle:
 *         Negotiate → Lock → Accept → Execute → Assert (OOv3) → Settle.
 *         Plan C: ServiceManager is the OOv3 asserter; only clients can dispute.
 */
contract ServiceManager is OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev Identity registry at slot 0 — still used by ServiceManager.
    ...
    bytes32 private constant SM_STORAGE_LOCATION =
        0xdafdb84b93a60307fbae8683888d5423afec5aa90d8e3d52c97325d829ff3c00;

    function _getSMStorage()
        private
        pure
        returns (ServiceManagerStorage storage $)
    {
        assembly {
            $.slot := SM_STORAGE_LOCATION
        }
    }
    ...
    // ============================================================
    //  Initializer (v3 — upgrade from EscrowUpgradeable v2)
    // ============================================================
    ...
    function initializeV3(
        ...
        ServiceManagerStorage storage $ = _getSMStorage();
    ...
    function oov3() external view returns (address) {
        return address(_getSMStorage()._oov3);
    }

    function paymentToken() external view returns (address) {
        return address(_getSMStorage()._paymentToken);
    }

    function acceptTimeout() external view returns (uint256) {
        return _getSMStorage()._acceptTimeout;
    }

    function submitTimeout() external view returns (uint256) {
        return _getSMStorage()._submitTimeout;
    }

    function oov3Liveness() external view returns (uint64) {
        return _getSMStorage()._oov3Liveness;
    }

    function bondRate() external view returns (uint256) {
        return _getSMStorage()._bondRate;
    }

    function minServiceFee() external view returns (uint256) {
        return _getSMStorage()._minServiceFee;
    }

    function nextJobId() external view returns (uint256) {
        return _getSMStorage()._nextJobId;
    }
    ...
    function pendingWithdrawals(address account) external view returns (uint256) {
        return _getSMStorage()._pendingWithdrawals[account];
    }
    ...
    /// @notice Rescue native BNB stuck in the contract (e.g. legacy EscrowUpgradeable payments).
